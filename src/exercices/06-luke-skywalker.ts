...
(() => {
  ...
  getRequest(`${SW_API}/people/1`)
    .pipe(
      ...
      // tap(console.log),
      // map((filmUrl) => {
      //   return getRequest(filmUrl);
      // }),
      // switchMap((observable$) => observable$)

      // switchMap((response) => getRequest(response.films[0])),
      switchMap((response) => zip(of(response), getRequest(response.films[0]))),
      map(([film, personaje]) => ({ film: film, personaje: personaje }))
    )
